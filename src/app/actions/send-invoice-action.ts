...
const formatDate = (date: any) => {
  ...
  return typeof date.toDate === 'function' ? date.toDate().toLocaleDateString() : new Date(date).toLocaleDateString();
};
...
export async function sendInvoiceByEmail(invoice: Invoice): Promise<{ success: boolean, message?: string, error?: string }> {
  ...
  try {
    ...
    const companyName = validatedData.siteIdentity?.siteName || 'BrandBoost AI';

    const issueDate = formatDate(validatedData.invoice.issueDate);
    const dueDate = formatDate(validatedData.invoice.dueDate);


    const emailResponse = await resend.emails.send({
      from: `${companyName} <onboarding@resend.dev>`,
      to: [validatedData.clientEmail],
      subject: `Invoice #${validatedData.invoice.invoiceNumber} from ${companyName}`,
      html: `
        <body style="font-family: Arial, sans-serif; margin: 0; padding: 20px; background-color: #f4f4f4; color: #333;">
          <div style="max-width: 600px; margin: auto; background-color: #ffffff; border-radius: 8px; padding: 40px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
            ...
            <p>Hi ${validatedData.invoice.clientName},</p>
            <p>A new invoice has been generated for your project, <strong>${validatedData.invoice.projectName}</strong>. Please review the details below.</p>
            <div style="border-top: 2px solid #e5e7eb; border-bottom: 2px solid #e5e7eb; margin: 20px 0; padding: 20px 0;">
              <table style="width: 100%; border-collapse: collapse;">
                <tr>
                  ...
                  <td style="text-align: right;">${validatedData.invoice.invoiceNumber}</td>
                </tr>
                <tr>
                ...
                <tr style="font-size: 24px; font-weight: bold;">
                  ...
                  <td style="padding-top: 20px; text-align: right; border-top: 1px solid #ccc;">${formatCurrency(validatedData.invoice.total)}</td>
                </tr>
              </table>
            </div>
            ${validatedData.invoice.paymentLink ? `<div style="text-align: center; margin: 30px 0;"><a href="${validatedData.invoice.paymentLink}" style="background-color: #6366f1; color: #ffffff; padding: 15px 25px; text-decoration: none; border-radius: 5px; font-weight: bold;">Pay Now Securely</a></div>` : ''}
    ...
    return { success: true, message: `Invoice sent to ${validatedData.clientEmail}` };
  } catch (error: any) {
